...
void scanPoint(int row, int col, int point_num, int total_points) {
  ...
  // 1. Tell Python we have arrived and stopped
  Serial.println("READY_FOR_CAM"); 
  
  // 2. Clear any old data in the buffer
  while(Serial.available() > 0) { Serial.read(); }

  // 3. Wait indefinitely until Python sends 'n'
  bool permission_to_move = false;
  while (!permission_to_move) {
    if (Serial.available() > 0) {
